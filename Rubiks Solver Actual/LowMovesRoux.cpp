...
LowMovesRoux::LowMovesRoux()
{
}

void LowMovesRoux::solveBottomLeft() {
...
void LowMovesRoux::solveBottomRight() {
	bool corner = false;
	int count = 0;
...
void LowMovesRoux::solveTopCorners() {
	...
	bool moveFiveJustDone = false;
	bool moveSixJustDone = false;
	while (!cornersInCorrectLocations) {
		...
		if (!frontRightCorrect && !backRightCorrect && !moveFourJustDone) {
			...
			moveFiveJustDone = false;
			moveSixJustDone = false;
			for (int i = 0; i < 4; i++) {
				bool correct = false;
				if (cubeToSolve.front.getPos(2, 0) == cubeToSolve.front.getPos(0, 2) || cubeToSolve.front.getPos(2, 0) == cubeToSolve.top.getPos(2, 2) || cubeToSolve.front.getPos(2, 0) == cubeToSolve.rightSide.getPos(0, 0)) {
					if (cubeToSolve.rightSide.getPos(2, 0) == cubeToSolve.front.getPos(0, 2) || cubeToSolve.rightSide.getPos(2, 0) == cubeToSolve.top.getPos(2, 2) || cubeToSolve.rightSide.getPos(2, 0) == cubeToSolve.rightSide.getPos(0, 0)) {
		...
		if (!frontLeftCorrect && !backRightCorrect && !moveFiveJustDone) {
			cubeToSolve.doR(2);
			cubeToSolve.doUPrime(1);
			cubeToSolve.doR(1);
			cubeToSolve.doFPrime(1);
		...
		if (!frontRightCorrect && !backLeftCorrect && !moveSixJustDone) {
			cubeToSolve.doU(1);
			cubeToSolve.doR(2);
			cubeToSolve.doUPrime(1);
			cubeToSolve.doR(1);
...
void LowMovesRoux::solveLastRedgeandLedge() {
	//last redge
...
void LowMovesRoux::completeCube() {
	for (int i = 0; i < 8; i++) {
...
bool LowMovesRoux::isSolutionValid() {
	bool valid = true;
...
void LowMovesRoux::setCube(RubiksCube cube) {
	cubeToSolve = cube;
}

RubiksCube LowMovesRoux::getCube() {
	return cubeToSolve;
}
