...
LowMovesCorners::LowMovesCorners() {
}

void LowMovesCorners::solveBottomCorners() {
	...
	while (!topCornersCorrect) {
		...
		//Should require less than 3 loops of algorithm
		int count = 3;
...
void LowMovesCorners::solveTopCorners() {
...
void LowMovesCorners::solveThreeLedges() {
	//left edges
	...
	while (!ledgesSolved) {
		...
		else if (cubeToSolve.top.getPos(1, 0) == cubeToSolve.leftSide.getPos(0, 0)) {
			cubeToSolve.doUPrime(1);
			cubeToSolve.doMPrime(1);
			cubeToSolve.doU(1);
		}
		else if (cubeToSolve.leftSide.getPos(0, 1) == cubeToSolve.leftSide.getPos(0, 0) && cubeToSolve.top.getPos(1, 0) != cubeToSolve.top.getPos(0, 0)) {
			cubeToSolve.doUPrime(1);
			cubeToSolve.doMPrime(1);
			cubeToSolve.doU(1);
		}
...
void LowMovesCorners::solveRedges() {
	//Right edges
...
void LowMovesCorners::solveLastLedge() {
	//last left edge
	for (int i = 0; i < 4; i++) {
...
void LowMovesCorners::flipMidges() {
...
void LowMovesCorners::completeCube() {
	for (int i = 0; i < 8; i++) {
...
bool LowMovesCorners::isSolutionValid() {
	bool valid = true;
...
void LowMovesCorners::setCube(RubiksCube cube) {
	cubeToSolve = cube;
}

RubiksCube LowMovesCorners::getCube() {
	return cubeToSolve;
}
